...
The `system.alerts` table contains insight alert configurations from your project. Alerts monitor insight values and notify subscribed users when thresholds are breached.
...
| `insight_id`           | ID of the insight being monitored                                                                        |
| `enabled`              | Whether the alert is actively evaluated (1 = yes, 0 = no)                                                |
| `state`                | Current state: `Firing`, `Not firing`, `Errored`, or `Snoozed`                                           |
| `calculation_interval` | How often the alert is checked: `hourly`, `daily`, `weekly`, or `monthly`                                |
| `condition`            | JSON containing the alert condition type (`absolute_value`, `relative_increase`, or `relative_decrease`) |
| `config`               | JSON containing trends-specific config (series index, check ongoing interval)                            |
...
| `snoozed_until`        | When the snooze expires (if snoozed)                                                                     |
| `skip_weekend`         | Whether to skip evaluation on weekends (1 = yes, 0 = no)                                                 |
