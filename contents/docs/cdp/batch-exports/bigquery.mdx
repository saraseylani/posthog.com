...
* **Integration:** Select a Google Cloud service account integration. Choose **Impersonate service account** for the recommended impersonation method (see [Option 1](#option-1-service-account-impersonation)), or **Upload service account JSON key file** if impersonation isn't available (see [Option 2](#option-2-json-key-file)).
...
See [Setting up BigQuery access](#setting-up-bigquery-access) for more information on configuring authentication.
