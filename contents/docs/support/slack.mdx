...
| **Bot appearance** | Custom display name and icon for the bot when posting messages. | Default bot name and icon |
||||||| 51429c429
| **Bot mentions** | Always active — `@SupportHog` in any channel creates a ticket from that message. | — |
=======
| **Bot mentions** | Always active – `@SupportHog` in any channel creates a ticket from that message. | – |
...
Set a custom display name and icon for the SupportHog bot when it posts to Slack.

1. Go to **Support > Settings**
2. Find the **Bot appearance** section
3. Enter a custom display name (e.g., "Acme Support")
4. Enter an icon URL (must start with `https://`)
5. Click **Save**

Leave these fields blank to use the default bot name and icon.

<CalloutBox icon="IconInfo" title="Reconnect required for older integrations" type="fyi">

If you connected SupportHog before this feature was available, you'll need to disconnect and reconnect Slack to grant the required permissions.
