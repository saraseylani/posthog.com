...
Signals are observations PostHog automatically generates by monitoring your data across error tracking, web analytics, experiments, session replay, and more. Each signal describes something noteworthy – like "Error rate spiked 3× on /checkout". You can ask your AI agent to search and analyze these signals.

**Prompt:** "Find signals related to checkout page issues"

The agent runs a semantic search over your project's signals, returning results ranked by relevance with details like description, source product, and timestamp.

**Prompt:** "What types of signals do I have?"

The agent queries your signals and returns a breakdown by source product (error tracking, web analytics, experiments, etc.) and signal type.

**Prompt:** "Show me recent signals from error tracking"

The agent filters signals by source product and returns the latest error tracking observations – like error rate spikes, new exception types, or affected user trends.
